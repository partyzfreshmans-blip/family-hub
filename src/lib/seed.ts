import { getDb, saveDatabase, queryOne } from './db';
...
export async function seedDatabase() {
  ...
  if (existing) {
    console.log('[SEED] Database already has seed data.');
    return;
  }
  ...
  // 1. Create Users
  const users = [
    { id: 'usr_dad', email: 'dad@familyhub.local', name: 'พ่อ (สมศักดิ์)', avatar: null },
    { id: 'usr_mom', email: 'mom@familyhub.local', name: 'แม่ (สุดา)', avatar: null },
    { id: 'usr_ton', email: 'ton@familyhub.local', name: 'น้องต้น', avatar: null },
    { id: 'usr_may', email: 'may@familyhub.local', name: 'น้องเมย์', avatar: null },
  ];
  ...
  for (const u of users) {
    db.run(
      `INSERT OR REPLACE INTO users (id, email, password_hash, display_name, avatar_url, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [u.id, u.email, passwordHash, u.name, u.avatar, now, now]
    );
  }

  // 2. Create Family: ครอบครัวสุขใจ
  ...
  db.run(
    `INSERT OR REPLACE INTO families (id, name, owner_id, currency, monthly_budget, rewards_enabled, avatar_icon, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [familyId, 'ครอบครัวสุขใจ', 'usr_dad', 'THB', 25000, 1, 'home', now, now]
  );
  ...
  for (const e of events) {
    db.run(
      `INSERT OR REPLACE INTO events (id, family_id, title, description, event_date, start_time, end_time, all_day, location, category, recurrence_rule, reminder_minutes, created_by, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [e.id, familyId, e.title, 'กิจกรรมครอบครัวสุขใจ', today, e.time, e.end, 0, e.loc, e.cat, 'NONE', 30, 'mem_dad', now, now]
    );
  ...
  for (const t of tasks) {
    db.run(
      `INSERT OR REPLACE INTO tasks (id, family_id, title, description, assigned_to, due_date, due_time, priority, status, recurrence_rule, points, created_by, completed_by, completed_at, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [t.id, familyId, t.title, 'งานบ้านประจำวัน', t.assign, today, t.time, t.prio, t.status, 'DAILY', t.pts, 'mem_dad', t.status === 'COMPLETED' ? t.assign : null, t.status === 'COMPLETED' ? now : null, now, now]
    );
  }
  ...
  for (const s of shopping) {
    db.run(
      `INSERT OR REPLACE INTO shopping_items (id, family_id, name, quantity, unit, category, note, added_by, purchased, purchased_by, purchased_at, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [s.id, familyId, s.name, s.qty, s.unit, s.cat, '', 'mem_mom', s.done, s.done ? 'mem_dad' : null, s.done ? now : null, now, now]
    );
  }

  // 8. Expenses
  const expenses = [
    { id: 'exp_1', amount: 1250, cat: 'Shopping', desc: 'ซื้อของ Lotus ซุปเปอร์มาร์เก็ต', paidBy: 'mem_dad', date: today },
    { id: 'exp_2', amount: 1850, cat: 'Utilities', desc: 'ค่าไฟประจำเดือน', paidBy: 'mem_mom', date: today },
    { id: 'exp_3', amount: 1000, cat: 'Transport', desc: 'เติมน้ำมันรถยนต์', paidBy: 'mem_dad', date: today },
    { id: 'exp_4', amount: 450, cat: 'Food', desc: 'มื้อกลางวันครอบครัว', paidBy: 'mem_mom', date: today },
  ];
  ...
  for (const exp of expenses) {
    db.run(
      `INSERT OR REPLACE INTO expenses (id, family_id, amount, category, description, paid_by, expense_date, note, created_by, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [exp.id, familyId, exp.amount, exp.cat, exp.desc, exp.paidBy, exp.date, 'ค่าใช้จ่ายครอบครัว', 'mem_dad', now, now]
    );
  }
  ...
  for (const b of bills) {
    db.run(
      `INSERT OR REPLACE INTO bills (id, family_id, name, amount, category, due_date, recurrence_rule, status, notes, created_by, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [b.id, familyId, b.name, b.amount, b.cat, b.due, b.repeat, b.status, 'ตัดผ่านบัญชี/สแกนจ่าย', 'mem_dad', now, now]
    );
  }
  ...
  for (const r of rewards) {
    db.run(
      `INSERT OR REPLACE INTO rewards (id, family_id, name, required_points, active, created_by, created_at)
       VALUES (?, ?, ?, ?, 1, ?, ?)`,
      [r.id, familyId, r.name, r.pts, 'mem_dad', now]
    );
  }
  ...
  for (const info of infos) {
    db.run(
      `INSERT OR REPLACE INTO household_info (id, family_id, category, title, value, contact_phone, notes, created_by, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [info.id, familyId, info.cat, info.title, info.val, info.phone, '', 'mem_dad', now]
    );
  }
